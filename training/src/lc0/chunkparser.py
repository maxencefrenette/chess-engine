...
import numpy as np
import random
import src.lc0.shufflebuffer as sb
import struct
import unittest
import gzip
...
class ChunkParserTest(unittest.TestCase):
    ...
    def test_parsing(self):
        ...
        parser = ChunkParser(
            ChunkDataSrc(records), shuffle_size=1, workers=1, batch_size=batch_size
        )
        batchgen = parser.parse()
        data = next(batchgen)

        batch = (
            np.reshape(np.frombuffer(data[0], dtype=np.float32), (batch_size, 112, 64)),
