...
def create_board_from_planes(
    planes_array: np.ndarray, castling_rights: List[int], side_to_move: bool = True
) -> chess.Board:
    """
...
def explore_lc0_file(filename: str, max_positions: Optional[int] = None):
    ...
    try:
        ...
        for features, best_q in parser.parse_chunk():
            ...
            # Create chess board
            board = create_board_from_planes(
                board_planes, castling_rights, side_to_move
            )

            # Display position information
            print(f"Position {position_count}:")
            print(
                f"Best Q: Win={best_q[0,0]:.4f}, Draw={best_q[0,1]:.4f}, Loss={best_q[0,2]:.4f}"
            )
            print(
...
def main():
    ...
    parser.add_argument(
        "input", help='Input LC0 chunk file (e.g., "/path/to/data/training.12345.gz")'
    )
    parser.add_argument(
