...
def test_lc0_to_npz_integration():
    ...
    try:
        ...
        assert features.shape[0] == best_q.shape[0], "Features and best_q have different numbers of positions"
        ...
        assert np.all(best_q >= 0) and np.all(best_q <= 1), "Best_q contains invalid probability values"
        
        # Sum of probabilities should be close to 1
        assert np.allclose(np.sum(best_q, axis=1), 1.0, atol=1e-5), "Win-Draw-Loss probabilities do not sum to 1"
